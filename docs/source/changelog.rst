...
- OpenSeesPy tutorial: a portal frame analyzed by an external finite-element
...
- **Generalized sensitivity parameters**: distributions can declare
...
- Developer guide: new section on adding distributions with
  sensitivity support.
- Theory docs: generalized parameter support section.
...
- **Type II Largest Value distribution**: corrected ``invweibull``
  parametrization (``c=k`` instead of ``c=-k-2``), which previously
