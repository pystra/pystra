...
def quadratureRule(n,wfun=None,alpha=None,beta=None):
  ...
  if wfun == 1:
    ...
    for j in range(iter):
      ...
      u=pk*dpn**(-1)
      v=d2pn*dpn**(-1)
      h=-u*(1+(.5*u)*(v+u*(v*v-u*d3pn*(3*dpn)**(-1))))
      p=pk+h*(dpn+(.5*h)*(d2pn+(h*3**(-1))*(d3pn+25*h*d4pn)))
      dp=dpn+h*(d2pn+(.5*h)*(d3pn+h*d4pn*3**(-1)))
      h=h-p*dp**(-1)
      xo=xo+h
    bp=-xo-h
    fx=d1-h*e1*(pk+(h*.5)*(dpn+(h*3**(-1))*(d2pn+(h*4**(-1))*(d3pn+(.2*h)*d4pn))))
    wf=2*(1-bp**2)*(fx*fx)**(-1)
    if ((m-1)+(m-1)) > (n-1):
      bp[m-1]=0
    if (m+m) != n:
