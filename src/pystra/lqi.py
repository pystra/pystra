"""Life quality index and societal willingness-to-pay helpers.
...
provide a small layer for turning failure probabilities into LQI acceptance
checks and simple cost-benefit studies.
"""
...
def _default_lqi_plot_quantities(data: pd.DataFrame) -> list[str]:
    candidates = [
        "pf",
        "annual_failure_rate",
...
def plot_lqi_summary(
    ...
    """Plot a one-dimensional LQI design-study summary.
    ...
    continuous one-dimensional design sweeps such as JCSS LQI examples.  More
    complex discrete alternatives or scenario studies can still use the same
    risk quantities, but usually need a problem-specific visualization.
    ...
    quantities : sequence of str, optional
        Result columns to plot.  When omitted, common LQI columns such as
    ...
    plot_quantities = (
        list(quantities) if quantities is not None else _default_lqi_plot_quantities(df)
    )
    if not plot_quantities:
        raise ValueError("No LQI quantities available to plot")
    ...
    try:
        ...
        raise ImportError("plot_lqi_summary requires matplotlib") from exc
